"""Fresh TRAIN-only scenario family for V14 direct-control learning."""
...
def v14_training_scenario(target_force_n: float, scenario_seed: int, scenario_id: int):
    from forcewipe_v4.scenarios import ScenarioSpec

    if float(target_force_n) not in {5.0, 8.0, 12.0}:
        raise ValueError("V14 target must be 5, 8, or 12 N")
    rng = np.random.default_rng(int(scenario_seed))
    spec = ScenarioSpec(
        scenario_id=int(scenario_id),
        role="TRAIN",
        seed_namespace="v5_train_v14_v6_direct_closed_loop_20260828",
