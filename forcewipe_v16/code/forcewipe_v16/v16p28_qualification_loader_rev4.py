"""Canonical V16.28 rev4 loader with disjoint qualification/CAL RNG domains."""
...
BUNDLE = ROOT / "v16_protocol" / "v16p28_paired_qualification_rev4"
BLOCKS_PATH = BUNDLE / "SCENARIO_BLOCKS_V16P28_R4.json"
TARGETS_N = (5.0, 8.0, 12.0)
SCENARIO_ID_BASE = 16_280_000
MAXIMUM_STEPS = 1200
QUALIFICATION_PLANNER_ROOT = 716_280_000
CAL_PLANNER_ROOT = 716_400_000
...
def _load_blocks() -> dict[str, dict]:
    ...
    if payload.get("format") != "forcewipe_v16p28_common_scenario_blocks_r4_v1":
        raise QualificationScenarioError("unexpected V16.28 rev4 block format")
    blocks = payload.get("blocks")
    if not isinstance(blocks, list) or len(blocks) != 5:
        raise QualificationScenarioError("V16.28 rev4 requires exactly five blocks")
    result = {str(block["block_id"]): block for block in blocks}
    if set(result) != {"Q0", "Q1", "Q2", "Q3", "Q4"}:
        raise QualificationScenarioError("V16.28 rev4 block roster is not Q0--Q4")
    return result
...
def _mixed_radix_seed(*, root: int, block_index: int, target_force_n: float, control_step: int, blocks: int) -> int:
    ...
    ordinal = (int(block_index) * len(TARGETS_N) + TARGETS_N.index(target)) * MAXIMUM_STEPS + int(control_step)
    return int(root) + ordinal
