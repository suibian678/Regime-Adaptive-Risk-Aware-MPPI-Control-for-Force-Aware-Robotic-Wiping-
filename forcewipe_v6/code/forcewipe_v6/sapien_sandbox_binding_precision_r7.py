"""Versioned physical binding for the precision-R7 controller."""
...
class PrecisionR7SurfacePathGeometry(S5SurfacePathGeometry):
    """Exact geometry identity for R7."""


class PrecisionR7SapienSandboxBinding(S5SapienSandboxBinding):
    """Issue and read back the inherited orthogonal R7 command."""

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        if type(self.supervisor) is not V6PrecisionContactSupervisorR7:
            raise V6SapienSandboxContractError(
                "precision-r7 binding requires the exact r7 supervisor"
            )
        if type(self.geometry) is not PrecisionR7SurfacePathGeometry:
            raise V6SapienSandboxContractError(
                "precision-r7 binding requires the exact r7 geometry"
            )
    ...
    def _build_issued_command(self, command, pre) -> IssuedCartesianCommand:
        ...
        if geometry is None or self._pre_by_key.get(pre.key) is None:
            raise V6SapienSandboxContractError(
                "precision-r7 issued command lacks pre-step geometry"
            )
        if (
            ...
            raise V6SapienSandboxContractError(
                "precision-r7 combined authority flags are inconsistent"
            )
        ...
        if np.any(np.abs(normalized) > 1.0 + 1e-12):
            raise V6SapienSandboxContractError(
                "precision-r7 combined Cartesian command would be clipped downstream"
            )
        command_id = ":".join(
            (
                pre.key.run_id,
