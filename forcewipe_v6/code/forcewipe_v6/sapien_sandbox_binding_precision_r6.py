"""Versioned physical binding for the precision-R6 controller."""
...
class PrecisionR6SurfacePathGeometry(S5SurfacePathGeometry):
    """Exact geometry type identity for the r6 development profile."""


class PrecisionR6SapienSandboxBinding(S5SapienSandboxBinding):
    """Issue/read back the inherited orthogonal command under r6 identity."""

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        if type(self.supervisor) is not V6PrecisionContactSupervisorR6:
            raise V6SapienSandboxContractError(
                "precision-r6 binding requires the exact r6 supervisor"
            )
        if type(self.geometry) is not PrecisionR6SurfacePathGeometry:
            raise V6SapienSandboxContractError(
                "precision-r6 binding requires the exact r6 geometry"
            )
    ...
    def _build_issued_command(self, command, pre) -> IssuedCartesianCommand:
        ...
        if geometry is None or self._pre_by_key.get(pre.key) is None:
            raise V6SapienSandboxContractError(
                "precision-r6 issued command lacks pre-step geometry"
            )
        if (
            ...
            raise V6SapienSandboxContractError(
                "precision-r6 combined authority flags are inconsistent"
            )
        ...
        if np.any(np.abs(normalized) > 1.0 + 1e-12):
            raise V6SapienSandboxContractError(
                "precision-r6 combined Cartesian command would be clipped downstream"
            )
        command_id = ":".join(
            (
                pre.key.run_id,
